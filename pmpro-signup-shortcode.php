 ...
 * Plugin URI: https://www.paidmembershipspro.com/wp/pmpro-signup-shortcode/
...
function pmprosus_pmpro_email_data( $data, $email ) {
	if ( ! empty( $_SESSION['pmprosus_autogenerated_password'] ) && strpos( $email->template, 'checkout_' ) !== false ) {
		$data['user_email'] = sprintf( __( 'Password: %1$s, Email: %2$s', 'pmprosus' ), $_SESSION['pmprosus_autogenerated_password'], $data['user_email'] );
	}
...
function pmprosus_signup_shortcode( $atts, $content = null, $code = '' ) {
	...
	?>
		...
		<form class="pmpro_form pmpro_signup_form
		<?php
		if ( ! empty( $hidelabels ) ) {
			?>
